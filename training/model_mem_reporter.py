"""
Borrowed from:
https://github.com/Stonesjtu/pytorch_memlab/blob/0d5f238c1bcfeb749efaa4337bbf1a89b481a8f9/pytorch_memlab/mem_reporter.py
"""
import gc
import logging
import math
...
from training.bytes_formatter import size as bytesformat
...
def readable_size(num_bytes):
    return f"{bytesformat(num_bytes):.2f}"
...
class MemReporter:
    ...
    def __init__(self, logger_name=None, model=None):
            ...
            for tensor in tensors:
                ...
                if isinstance(tensor, torch.nn.Parameter):
                    if tensor.grad is not None:
                        ...
                        self.tensor_name[tensor.grad] = '{}.grad'.format(self._get_tensor_name(tensor))
    ...
    def log_stats(self, report_name, verbose=False, target_device=None):
        # header
        show_reuse = verbose
        self.logger.debug(f"################################# {report_name} #################################\n")
        template_format = '{:<40s}{:>20s}{:>10s}'
        self.logger.debug(template_format.format('Element type', 'Size', 'Used MEM'))
        for device, tensor_stats in self.device_tensor_stat.items():
            # By default, if the target_device is not specified,
            # print tensors on all devices
            if target_device is not None and device != target_device:
            ...
            self.logger.debug('Storage on {}'.format(device))
            total_mem = 0
            total_numel = 0
            for stat in tensor_stats:
                name, size, numel, mem = stat
                if not show_reuse:
                    name = name.split('(')[0]
                self.logger.debug(template_format.format(
                    str(name),
                    str(size),
                    readable_size(mem),
                ))
            ...
            self.logger.debug('-' * LEN)
            self.logger.debug('Total Tensors: {} \tUsed Memory: {}'.format(
                total_numel, readable_size(total_mem),
            ))
            ...
            if device != torch.device('cpu'):
                with torch.cuda.device(device):
                    memory_allocated = torch.cuda.memory_allocated()
                self.logger.debug('The allocated memory on {}: {}'.format(
                    device, readable_size(memory_allocated),
                ))
                if memory_allocated != total_mem:
                    self.logger.debug('Memory differs due to the matrix alignment or'
                                      ' invisible gradient buffer tensors')
            self.logger.debug('-' * LEN)
    ...
    def log_report(self, report_name="DEFAULT REPORT NAME", verbose=False, device=None):
        """Interface for end-users to directly print the memory usage
        args:
            - verbose: flag to show tensor.storage reuse information
            - device: `torch.device` object, specify the target device
        ...
        self.collect_tensor()
        self.get_stats()
        self.log_stats(report_name, verbose, target_device=device)
